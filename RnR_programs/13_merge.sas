...
%LET analysis=aalb;
%setup(programName=013_merge_&analysis, savelog=Y, dataset=temp);
    ...
    *stacking the exposure and comparator datasets together, coded to keep duplicate rows and only same columns;
    ...
    /* Baseline vars- crude  */
        ...
        *  label duration_metformin="Proxy for duration of treated DM: Days between first date of metformin rx and (excl) cohort entry date";
		...
		%END;
		%ELSE %IF &analysis=1yrlb %THEN %DO;
    ...
    %if &save=Y %then %do; 
    data temp.allmerged_&exposure._&comparator._&analysis;
    set tmp1;RUN;
    data temp.excl_013_&exposure._&comparator._&analysis;
    set tmp_counts;RUN;
    %end;
%end;
%mend mergeall;
...
%LET comparatorlist = su tzd sglt2i;
/*7/25/2024, Tian: ran comparatorlist separately otherwise got errors*/
...
proc contents data= temp.allmerged_dpp4i_su_&analysis varnum; run;
ods excel options(sheet_name="dpp4i_su counts" sheet_interval="NOW");
proc print data=temp.excl_013_dpp4i_su_&analysis; run;
ods excel options(sheet_name="dpp4i_tzd contents" sheet_interval="NOW");
proc contents data= temp.allmerged_dpp4i_tzd_&analysis varnum; run;
ods excel options(sheet_name="dpp4i_tzd counts" sheet_interval="NOW");
proc print data=temp.excl_013_dpp4i_tzd_&analysis; run;
...
proc contents data= temp.allmerged_dpp4i_sglt2i_&analysis varnum; run;
