...
option SASAUTOS=(SASAUTOS "D:\Externe Projekte\UNC\wangje\prog\sas\macros")
		...
		macrogen nosymbolgen nomlogic nomprint mcompile mcompilenote=all MAUTOSOURCE;
...
%macro psweighting ( exposure , comparator , weight , addedmodelvars ,basemodelvars , tablerowvars, refyear  , dat, save, ana_name);
    ...
    *  ods rtf file="&goutpath./&todaysdate.psoutput&exposure._&comparator..rtf";
    proc logistic data=tmp1 descending;
        class  entry_year (ref="&refyear.") sex (ref=first) hba1c_Cat2 (ref=first) alc (ref=first)
 ...
 ods pdf close;
    ...
    * Univariate analyses on weight varialbes by treatment status checking for extreme weights;
    ...
    proc logistic data=psdsn desc;
        class entry_year (ref="&refyear.") sex (ref=first) hba1c_Cat2 (ref=first) alc (ref=first) smoke (ref=first) bmi_cat2(ref=first) /param=ref;
    ...
    * check univariate analysis on weight variables by treatment status, check for extreme weights;
    ...
    %if &save. = Y %then %do;
        ...
        data a.notrim_&exposure._&comparator._1yrlb; set psdsnnotrim; run;
        /* Saving the PS Trimmed Cohort for sensitivity analysis #3 */
        data a.PS_&exposure._&comparator._1yrlb; set psdsn; run;
        /* Updating exclusions for PS trimming */
        PROC SQL; 
            create table tmp_counts as select * from temp.excl_014_&exposure._&comparator._1yrlb;
            ...
            create table temp.excl_015_&exposure._&comparator._1yrlb as select * from tmp_counts;
        QUIT;
    %end;%else %do; %end;
...
/*7/21/2024 Jeanny & Tian maybe using 1-year LL for drugs*/
/*Alc and smoke uses all available lookback*/
%LET basevars =  age|age
...
%include "D:\Externe Projekte\UNC\wangje\prog\sas\macro_psweighting_Ab.sas";
/*need to do All avaialbe lookback to replicate Abrahami study, also using 1year lookback has % is not close to % by Abrahami!!!!*/
%LET tablerowvarsi = age
sex entry_year   
