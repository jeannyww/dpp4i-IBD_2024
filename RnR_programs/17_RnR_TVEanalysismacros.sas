...
Date: 2024-04-21
Notes: Added AT analysis in lines 1046-1064 
...
title "tmp_counts"; proc print data=tmp_counts;run;title;
    ...
    PROC SQL NOPRINT; 
        ...
        * Had the following diagnosed diseases before the first prescription were excluded: (a-f non-mutually exclusive)) ;
        ...
        dpp4i            = (select count(*) from tmp2 where dpp4i=1 and delete_IBD ne 1 and (PCOS_bl in (.,0) and DiabGest_bl in (., 0))) , 
        dpp4i_diff       =-(select count(*) from tmp2 where dpp4i=1 and delete_IBD eq 1 and (PCOS_bl not in (.,0) and DiabGest_bl not in (., 0)))  ,
        &comparator      = (select count(*) from tmp2 where dpp4i=0 and delete_IBD ne 1 and (PCOS_bl in (.,0) and DiabGest_bl in (., 0))),
        &comparator._diff=-(select count(*) from tmp2 where dpp4i=0 and delete_IBD eq 1 and (PCOS_bl not in (.,0) and DiabGest_bl not in (., 0)));
...
/* Jeanny Addition for missing criteria */
proc sql noprint;
    select count(*) into : num_obs from tmp_counts;
    insert into tmp_counts
        set exclusion_num=&num_obs+1, 
...
/*===================================*\
    cox regression 8/10/2024 Tian added, 
but realized this need to be done in PS untrimmed cohort (psdsnnotrim), need to do first get PS untrimmed cohort, then get event, time, then run cox regression.
need to focus on my job talk, thus, will give up cox regression for now. 
\*===================================*/
/*ods output ParameterEstimates = mvcoxhr;
proc phreg data=dsn covsandwich(aggregate) ;
   class    sex alcohol_cat (ref=first)  smoke_cat  (ref=first)  hba1c_cat2   (ref=first)  bmi_cat  (ref=first) ;
   model time*event(0)= &exposure.
age*age &tablerowvarsi.
/ties=efron rl;
run;
ODS rtf FILE="&toutpath./mvcoxhr_&exposure.v&comparator._&ana_name..rtf" options(orientation='landscape');
