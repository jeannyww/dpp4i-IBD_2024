...
Date: 2025-01-01
Changes: Organized macro for more detailed and intuitive table
***************************************/
...
%macro ACNU_analysis (pstrim, exposure , comparator , ana_name , type , 
weight , induction , latency , ibd_def , intime , outtime, numyears ,
outdata, save) / minoperator mindelimiter=',';
...
%if %upcase(&pstrim.) eq Y %then %do; 
    ...
    data dsn; set a.PS&ana_name._&exposure._&comparator._1yrlb; run;
%end; %else %if %upcase(&pstrim.) eq N %then %do;
    /* Untrimmed for main analysis now */
    data dsn; set a.notrim&ana_name._&exposure._&comparator._1yrlb; run;
%end;
...
data dsn; set dsn;format dpp4i;  if delete_flag1=1 then delete;
 run;

/*Then get numbers for the Results Table*/
/*1) Get sum of IBD events by dpp4i*/
...
quit;
/*2) Get sum and median followup byDPP4i*/
...
quit;
/*3) Get sum of and median time on drug by dpp4i*/
...
/*4) to 7) intermediate steps in %TVE_analysis() macro are not done in ACNU because not accounting for switching*/
/*8) get Incident Rates*/
data dsn; set dsn; format dpp4i; run;
proc sort data=dsn; 
	by &exposure; 
run;
%let exposure=dpp4i;
...
/*Wide version*/
data summaryACNU_W_&exposure._&comparator.;
merge &exposure._results (drop = crude_HR smrw_HR) &comparator._results ; 
analysis = "ACNU &type. &ana_name.";
run;
title "Summary results &exposure. &comparator."; 
proc print ; run; 
/*Long version*/
proc sql;
create table summaryACNU_L_&exposure._&comparator.
as select * from 
	(select 
	"ACNU &type. &ana_name." as analysis, 
	. as switcher_flag,
	"&exposure." as drug, 
	status, 
...
select * from summaryACNU_L_&exposure._&comparator.;
quit;
title ""; 
...
proc sql;
select * from summaryACNU_W_&exposure._&comparator.;
quit;
...
STACKODS N NMISS SUM MEAN STD MIN MAX Q1 MEDIAN Q3 maxdec=2  ; 
where delete_flag1 eq 0;
class dpp4i ibd_ever;  *important to use switcher_flag, not switcher variable;
var    time    event    ; 
run;
/* endregion //!SECTION */
...
%macro summarize(exposure, comparator);
/*Long version*/
/*ACNU*/
title "Summary output Long version &exposure. v &comparator.";
proc sql;
...
/*stack long versions*/
proc sql;
create table summary_L_&exposure._&comparator.
as select * from 
 summaryACNU_L_&exposure._&comparator.
union all corr
(select * from 
summaryTVE_L_&exposure._&comparator.);
quit;
/*Wide Versions*/
/*ACNU*/
title "Summary output Wide Version &exposure. v &comparator."; 
...
quit;
data summary_W_&exposure._&comparator.; retain analysis;
length status $20.;
set summaryACNU_W_&exposure._&comparator. summaryTVE_W_&exposure._&comparator.;
run;
/* Final print of both output */
title "Summary output Wide Version &exposure. v &comparator."; 
proc print data= summary_W_&exposure._&comparator.;
run;
title "Summary output Long Version &exposure. v &comparator."; 
proc print data=summary_L_&exposure._&comparator.; run; 
