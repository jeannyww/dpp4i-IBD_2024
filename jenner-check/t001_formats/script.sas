...
/* jenner-check addition: apply the formats above to a small sample of
   cohort-shaped rows so the run produces a visible listing (the
   proc format block above is unmodified from prog/sas/formats/formats.sas). */
data cohort_sample;
	length sex $1 alcohol_status $1;
	input id sex $ alcohol_status $ bmi_cat hba1c_cat tx_at_timepoint;
	format sex $sexf. alcohol_status $statusf. bmi_cat bmif. hba1c_cat hba1cf. tx_at_timepoint tx_at_timepf.;
	datalines;
1 f u 1 1 0
2 m s 2 2 8
3 f c 3 3 12
...
proc freq data=cohort_sample;
	tables sex alcohol_status bmi_cat hba1c_cat tx_at_timepoint / missing;
run;
