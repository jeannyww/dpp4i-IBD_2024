...
PROC FREQ DATA=dsn_TVE; 
WHERE deleteobs=0 and IBDdx_inductionperiod=0 and time ne .;
TABLES dpp4i*trimming_flag*keepflag_prevalentuser*excludeflag_prevalentuser*&ibd_def*event*IBD_EVER /list missing;
RUN;
...
* Uncomment to first print and manually inspect the data to check that events were attributed correctly to DPP4i and Comparator person time. ; 
/* *Expect to print out 391 rows;
proc print data=ACNU_su_dpp4i; run;
*Expect to print out 456 rows;
proc print data=TVE_su_dpp4i; run; */
...
/* 49 IBD censored  */
proc print data=TVE_su_dpp4i; 
where  (event eq 0) and (switchAugmentDate ne .) and (dpp4i eq 0);  
RUN;
/* count events missed due to events being attributed to Dpp4i initiators who were prevalent users of the comparator  (events that would have been in the comparator's person time as it would be in our Main IT analysis if not for the censoring at 180+switch/augment/fill2date date )*/
ods output summary= IBD_events_censored;
Proc means data=TVE_su_dpp4i sum stackods ;
...
PROC SQL NOPRINT;
    create tmp1 as 
    select distinct id, filldate2 from dsn_TVE
