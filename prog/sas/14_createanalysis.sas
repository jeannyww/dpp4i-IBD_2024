...
%setup(programName=14_createanalysis, savelog=Y, dataset=dataname);

%macro createana(exposure=, comparatorlist=, save=Y);
...
/* if save==y then save to analysis folder */
    %if &save=Y %then %do;
    data a.allmerged_&exposure._&comparator._1yrlb;
        set  tmpana_&exposure._&comparator.;
        RUN;

    data temp.excl_014_&exposure._&comparator._1yrlb;
        set tmp_counts;
        RUN;
    %end;
...
ods excel file="&toutpath./Testcounts_1yrlb.xlsx"
  options (
    Sheet_interval="PROC"
    embedded_titles="NO"
...
proc print data=temp.excl_014_dpp4i_su_1yrlb; run;
ods excel options(sheet_name="dpp4i_tzd counts" sheet_interval="NOW");
proc print data= temp.excl_014_dpp4i_tzd_1yrlb; run;
ods excel options(sheet_name="dpp4i_sglt2i counts" sheet_interval="NOW");
proc print data=temp.excl_014_dpp4i_sglt2i_1yrlb;
run;
