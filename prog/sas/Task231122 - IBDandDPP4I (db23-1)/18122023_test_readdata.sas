...
/* testing the output of proc import to see if the formats are in congruent with the infile method  */
proc contents data=raw.dpp_event2; run;  *this version is the PROC IMPORT method ; 
proc contents data=raw.dpp_event; run; *this version is from the infile method ;
...
%macro read_eventcsv ( druglist  );
    ...
    data raw.&drug._event; 
        ...
        length ID $ 12 eventtype 8 EventDate $ 10 EventDate_tx $ 10 badrx_BCode $ 10 badrx_GCode $ 10 EndOfLine $ 10;
        input ID $ eventtype ReadCode $ EventDate $ EventDate_tx $ badrx_BCode $ badrx_GCode $ EndOfLine $;
    run;
...
/* reading in these datasets took max 1 second per drug, compared to at 10-15 minutes for one proc import */

/* testing the creation of this macro  */
proc contents data=raw.dpp4i_event; run; *this version is from the infile method ;
...
%read_trtmtcsv ( sglt2i );

%LET druglist = dpp4i  tzd su sglt2i ;
%read_trtmtcsv ( &druglist. );
/* endregion //!SECTION */
