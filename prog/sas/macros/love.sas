...
Goal: To create a figure that shows covariate imbalance across levels of a binary treatment/exposure before and after adjustment by weighting or matching.
...
Steps:
    ...
    3. Specify the three required input parameters for the %love macro (i-iii):
	    ...
	    i. The WORKING_DIRECTORY parameter: Specify the directory that contains the input dataset and 		where the final plots will be stored.

	    ii. The SORTCHOICE parameter has two specification options, "sorted" or "original" (not case sensitive):

	        SORTED: The figure will display covariates in descending magnitude of crude standardized mean difference.
	...
	4. If desired, two optional parameters may be modified in the macro call. These will receive default values if not modified.
	    ...
	    iv. The THRESHOLD parameter: Specify the absolute value of your threshold for identifying potentially meaningful covariate imbalance. This will generate reference lines in the figure. 
		If you do not want 	threshold reference lines in the figure, then specify a value of threshold=0 in your call of the %love 	macro. The default threshold is 0.1.

	    v. The HORIZONTAL parameter has two recommended specification options, "bands" or "lines" (case sensitive).

	        bands: This default setting will use light gray horizontal bands to guide the reader's eye 	horizontally across the figure from covariate names to their standardized mean differences.
